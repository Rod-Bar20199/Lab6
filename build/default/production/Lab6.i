...
# 33 "Lab6.s" 2
 ...
 RESET_TMR1 MACRO TMR1_H,TMR1_L
    ...
    MOVLW TMR1_L
    MOVWF TMR1L
    BCF ((PIR1) and 07Fh), 0 ; Limpiamos la bandera de interrupción del Timer .
    ENDM
...
ISR:
    ...
    BTFSC ((INTCON) and 07Fh), 2 ; Se verifica la bandera del TMR0
...
INT_TMR1:
    ...
    INCF PORTA
                          ...
                          ; Movemos el valor del registro al PORTA
    RETURN
...
CONFIG_TMR0:
   ...
   BCF ((OPTION_REG) and 07Fh), 5 ; Establecemos el Timer0 como temporizador
   BCF ((OPTION_REG) and 07Fh), 3 ; Prescaler a TIMER0
   BSF ((OPTION_REG) and 07Fh), 2 ; ((OPTION_REG) and 07Fh), 2 = 1
   BCF ((OPTION_REG) and 07Fh), 1 ; ((OPTION_REG) and 07Fh), 1 = 1
   BSF ((OPTION_REG) and 07Fh), 0 ; ((OPTION_REG) and 07Fh), 0 Prescaler de 1 : 256

   BANKSEL TMR0 ; Cambiamos de banco
   MOVLW 252 ; Valor cargado para obtener un retraso de 2 ms.
   MOVWF TMR0
   BCF ((INTCON) and 07Fh), 2 ; Se limpia la bandera de interrupción
   RETURN
; Cada vez que se cumple el tiempo del timer0 se debe de reiniciar

CONFIG_TMR1:
    BANKSEL T1CON
    BCF ((T1CON) and 07Fh), 1 ; Usar reloj interno
    BCF ((T1CON) and 07Fh), 3 ; Apagamos el oscilador de baja frecuencia
    BSF ((T1CON) and 07Fh), 5 ; Prescaler 1:8
    BSF ((T1CON) and 07Fh), 4
    BCF ((T1CON) and 07Fh), 6 ; Timer siempre contando
    BSF ((T1CON) and 07Fh), 0 ; Encendemos el Timer1
...
CONFIG_TMR2:
 ...
 CONFIG_IN_OUTS:
    ...
    BCF TRISD, 0 ; ((PORTD) and 07Fh), 0 como salida para display
    BCF TRISD, 1 ; ((PORTD) and 07Fh), 1 como salida para display
    BANKSEL PORTD
...
CONFIG_INTERRUPCIONES:
    ...
    BANKSEL INTCON
    BSF ((INTCON) and 07Fh), 6 ; Habilitamos interrupciones de periféricos
    BSF ((INTCON) and 07Fh), 7 ; Habilitamos interrupciones globales
    BSF ((INTCON) and 07Fh), 5 ; Habilitamos interrupcion TMR0
    BCF ((INTCON) and 07Fh), 2 ; Limpiamos bandera de TMR0
    BCF ((PIR1) and 07Fh), 0 ; Limpiramos la bandera del TMR1
...
obtener_numero:
    ...
    decenas:
        ...
        BTFSS STATUS,0 ; 10 > valor, Bandera de Carry apagada, C=0
 ...
 MOVWF valor
    ...
    unidades:
        ...
        BTFSS STATUS,0 ; 1 > valor, Bandera de Carry apagada, C=0
